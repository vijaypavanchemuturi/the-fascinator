...
class DetailData:
    ...
    def hasOgg(self):
        if self.__oggFlag is not None:
            return self.__oggFlag
        altPreviews = self.getAltPreviews() 
        if altPreviews:
            for altPreview in altPreviews:
                try:
                    payload = self.getObject().getPayload(altPreview)
                    mimeType = payload.getContentType()
                    if mimeType == "application/x-ogg":
                        self.__oggFlag = urllib.quote(payload.getId())
                        return self.__oggFlag
                except StorageException, e:
                    pass
        self.__oggFlag = False
        return False
