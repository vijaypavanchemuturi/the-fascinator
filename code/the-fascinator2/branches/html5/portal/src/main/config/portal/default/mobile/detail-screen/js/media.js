...
$(function() {
    ...
    var filename = "$self.getObject().getSourceId()";
    var hasOgg = "$self.hasOgg()";
    alert ("here");
    if (hasOgg!='') {
    	var href = "$portalPath/download/$oid/" + filename;
        var player1 = '<video href="' + href + '" id="player"></video>'; 
        $("#player-container").attr("style", "width: 300px; height: " + height + "px").html(player1);
    } 
    ...
    var hasOgg = "$self.hasOgg()";
    if (hasOgg != '') {
        var source = "$portalPath/download/$oid/" + filename;
        var href = "$portalPath/download/$oid/" + hasOgg;
        var player1 = "<video id=player controls width=300 height=" + height + ">" +
                       "<source src=" + href + "/>" +
                     +"</video>";
        //"<source src=" + filename + "/>" +
        $("#player-container").attr("style", "width: 300px; height: " + height + "px").html(player1);
