...
function callback(data){var href,anchors={};var pageToc=jQ(data).find("div.page-toc");var body=jQ(data).find("div.body");if(body.size()==0){pages[hash]=data;rvt.updateContent(data);rvt.contentScrollToTop();return;}
body.find("div.title").show().after(pageToc);if(rvt.fixRelativeLinks){body.find("a").each(function(c,a){a=jQ(a);href=a.attr("href");if(href){if(href.substring(0,1)==="#"){a.attr("href","#"+hash+href);anchors[href.substring(1)]=hash+href;}else{if(href=getHPath(href)){a.attr("href","#"+href);}}}});}
body.find("a").each(function(c,a){var id,name;a=jQ(a);id=a.attr("id");name=a.attr("name");if(anchors[id]){a.attr("name",anchors[id]);}else if(anchors[name]){a.attr("name",anchors[name]);}});var baseUri="";if(rvt.fixRelativeLinks){var a=window.location.hash.split("#",2).slice(1)[0];baseUri=rvt.contentBaseUrl;if(baseUri[baseUri.length-1]!=="/")baseUri+="/";baseUri+=a.split("/").slice(0,-1).join("/");if(baseUri[baseUri.length-1]!=="/")baseUri+="/";}
function updateUri(node,attrName){var attribute=node.attr(attrName);function testIfLocalUri(uri){if(!uri){return false;}
return uri.search(/^\/|\:\/\//g)===-1;}
if(testIfLocalUri(a)){node.attr(attrName,baseUri+attribute);}}
body.find("*[src]").each(function(c,node){node=jQ(node);updateUri(node,"src");});body.find("object").each(function(c,node){node=jQ(node);updateUri(node,"data");updateUri(node,"codebase");node.find("param[name='movie'],param[name='url'],param[name='media']").each(function(c,node){node=jQ(node);updateUri(node,"value");});});var html=body.find(">div");jQ(data).find("div.rendition-links").each(function(c,node){html.prepend("<div class='rendition-links'><span class='heading'>Renditions</span>"+unescape(jQ(node).html())+"</div>");});pages[hash]=html;rvt.updateContent(pages[hash]);rvt.contentScrollToTop();}
