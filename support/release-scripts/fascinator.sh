#!/bin/sh
#
# Undertakes a Maven release of the Fascinator POM
# Also creates a maven site for the version
#
# Usage: ./fascinator.sh <release-name> <release-version> <new-snapshot-version>
#
# Where version is the SVN tag name that will be used.
#   For example: ./fascinator.sh fascinator 0.2.0 0.3.0-SNAPSHOT 

RELEASE_NAME=$1
VERSION=$2
SNAPSHOT=$3
BASE_URL=https://fascinator.usq.edu.au/svn-auth/code/the-fascinator2
...
#Prepare for release
if ! mvn --batch-mode -Dtag=$RELEASE_NAME-$VERSION release:prepare -DreleaseVersion=$VERSION -DdevelopmentVersion=$SNAPSHOT -DautoVersionSubmodules=true; then
